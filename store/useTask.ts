		...
		addItem(boardName: string, members: string[]) {
			...
			if (process.client) {
				...
				if (itemForManageMent === "board") {
					...
					const boardId = "board-1"; // add columnIds
					const columnId = `column-${Math.floor(Math.random() * 20000000000)}`;
					const columnName = "Sprint 1 campaign";
					const findBoardIndex = this.boards.findIndex((board) => board.boardId === boardId);
					const mutableFindBoard = { ...this.boards[findBoardIndex] };
					console.log(mutableFindBoard, "findBoardIndex");
					mutableFindBoard.columnIds = [...mutableFindBoard.columnIds, columnId];
					console.log(mutableFindBoard.columnIds, "columnIds");
					console.log(mutableFindBoard, "after added everthinh");

					// this.columns.push({ columnId: columnId, columnName: columnName, taskIds: [] });
